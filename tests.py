...
class TestCase(unittest.TestCase):
    ...
    def test_follow_posts(self):
        ...
        u1 = User(nickname='david', email='david@example.com')
        db.session.add(u1)
        db.session.add(u2)
        db.session.add(u3)
        db.session.add(u4)
        ...
        db.session.commit()
        # setup the followers
        u1.follow(u1)   # john follows john
        u1.follow(u2)   # john follows susan
        u1.follow(u4)   # john follows david
        u2.follow(u2)   # susan follows susan
        u2.follow(u3)   # susan follows mary
        u3.follow(u3)   # mary follows mary
        u3.follow(u4)   # david follows mary
        u4.follow(u4)   # david follows david
        db.session.add(u1)
        db.session.add(u2)
        db.session.add(u3)
        db.session.add(u4)
